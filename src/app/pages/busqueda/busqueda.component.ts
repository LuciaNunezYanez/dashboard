...
import { ContactoEmergencia } from 'src/app/services/utilidades/interfaces';
...
export class BusquedaComponent implements OnInit {
  ...
  // Reporte individual
  objReporteFull: any = [];
  objContactos: ContactoEmergencia[];
  objMultimedia: any = [];
  objActivaciones: any = [];
  objImagenes: any = [];
  objAudio: any = [];
  ...
  verReporte(folio: number) {
    ...
    this._busq.getReporteFull(folio).subscribe((results: any) => {
      ...
      if (results.reporteFull) {
        ...
        this.objContactos = results.contactos;
        ...
        // Traer la foto de perfíl 
        if(this.objReporteFull.id_foto_perfil_app > 0){
          this._obtener.getRutaImgPerfil(this.objReporteFull.id_foto_perfil_app).subscribe((data_imagen: any)=>{
